...
def train_epoch(model, training_data, optimizer, opt, device, smoothing):
    ...
    for batch in tqdm(training_data, mininterval=2, desc=desc, leave=False):
        ...
        # Add grident cliping to avoid big rate of lr increasement.
...
def train(
    ...
    print(
        "✅  Training performance will be written to file: {} and {}".format(
            log_train_file, log_valid_file
        )
    )

    file_mode = "a" if start_epoch > 0 else "w"
    with open(log_train_file, file_mode) as log_tf, open(
        log_valid_file, file_mode
    ) as log_vf:
        if start_epoch == 0:
    ...
    for epoch_i in range(start_epoch, opt.epoch):
        ...
        checkpoint = {
            ...
            "optimizer_n_steps": optimizer.n_steps,
            "valid_losses": valid_losses,
        }

        if opt.save_mode == "all":
            all_model_name = "model_accu_{accu:3.3f}.chkpt".format(
                accu=100 * valid_accu
            )
            torch.save(checkpoint, all_model_name)
...
def main():
    ...
    if not opt.output_dir:
        print("⚠️ No experiment result will be saved.")
        raise
    ...
    if opt.batch_size < 2048 and opt.n_warmup_steps <= 4000:
        print(
            "⚠️  The warmup steps may be not enough.\n"
    ...
    if os.path.isfile(CHECKPOINT_PATH):
        print("✅  Found existing checkpoint: {}".format(CHECKPOINT_PATH))
        checkpoint = torch.load(
            CHECKPOINT_PATH, map_location=device, weights_only=False
        )
        # backward compatibility: old format used key "model"
